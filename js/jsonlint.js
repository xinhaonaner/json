                        ...
                        for (;;) {
                            ...
                            if (typeof t == "undefined" || !t.length || !t[0]) {
                                if (!k) {
                                    ...
                                    var B = "";
                                    this.lexer.showPosition ? B = "鍦ㄧ"+ (i + 1)+"琛屽彂鐢熻В鏋愰敊璇� "+ ":<br/>" + this.lexer.showPosition() + "<br/>姝ゅ缂哄皯" + A.join(", ") + "瀛楃, 瀹為檯涓婄‘鏄竴涓� '" + this.terminals_[q] + "'" : B = "鍦ㄧ"+ (i + 1)+"琛屽彂鐢熻В鏋愰敊璇� " + ": 鏈簲璇ユ槸 " + (q == 1 ? "缁撳熬杈撳叆" : "'" + (this.terminals_[q] || q) + "'"), this.parseError(B, {
                                ...
                                if (k == 3) {
                                    if (q == m) throw new Error(B || "瑙ｆ瀽鎰忓缁堟.");
                                    j = this.lexer.yyleng, h = this.lexer.yytext, i = this.lexer.yylineno, n = this.lexer.yylloc, q = p()
                                }
                                for (;;) {
                                    if (l.toString() in g[s]) break;
                                    if (s == 0) throw new Error(B || "瑙ｆ瀽鎰忓缁堟.");
                                    o(1), s = d[d.length - 1]
                                }
                                r = q, q = l, s = d[d.length - 1], t = g[s] && g[s][l], k = 3
                            }
                            if (t[0] instanceof Array && t.length > 1) throw new Error("瑙ｆ瀽閿欒: multiple actions possible at state: " + s + ", token: " + q);
                            switch (t[0]) {
                            case 1:
                    ...
                    b = function() {
                        var a = {
                            ...
                            next: function() {
                                ...
                                this.parseError("璇嶆眹閿欒鍙戠敓鍦ㄧ" + (this.yylineno + 1) + "琛�. 涓嶈兘璇嗗埆鐨勫瓧绗�.<br/>" + this.showPosition(), {
                                    text: "",
                                    token: null,
                                    line: this.yylineno
                                })
